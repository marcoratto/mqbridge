...
public class QueueToFile {
	...
	public static void main(String[] args) {
		...
		QueueToFile instance = new QueueToFile();
		int retCode = -1;
		try {
			instance.runme(args);
			retCode = 0;
	...
	public void runme(String[] args) throws FileToQueueException {
		...
		mqUtility.setOutputEncoding(this.encoding);
		
		String buffer = null;
		try {
			buffer = mqUtility.read();

			if (this.base64Encoding) {
				buffer = Base64.decode(buffer);
			}

			Utility.stringToFile(file, this.encoding, buffer);		
